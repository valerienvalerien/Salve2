...
| 6 | Pas de `Referrer-Policy` : l'URL pouvait fuiter vers un tiers via un clic sortant. Critique maintenant que l'URL contient un token. | 🟠 | ✅ `no-referrer` en meta + en-tête `.htaccess` (Apache/OVH) |
...
`tools/build-site.sh` publie `espace/*.html` avec `noindex/no-store/no-referrer` (via
`.htaccess` généré au build, uploadé sur OVH), et **échoue** si un contenu de deal non
chiffré s'y retrouve.
